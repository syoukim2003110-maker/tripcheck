...
public struct MapPin: Identifiable, Equatable, Sendable {
  ...
  /// **停留所の id そのもの**。詳細シートを開く鍵(`Inspector.stop(id)`)であり、
  /// 強調の照合先(`view.mapFocusedStopId`)でもあるので、ここに別の id を作らない。
  ...
  /// 丸の中に出る字 —— 訪問の番号。絵で名乗る種別(食事・ホテル・提案・手動)は**空**で、
  /// `PinView` が代わりにアイコンを置く。
  public var label: String
  /// 「ツェルマット、予定地点」。地図の点は絵なので、名前と種別はこの 1 文だけが運ぶ。
  public var a11y: String

  public init(id: String, coordinate: GeoPoint, kind: Kind, dayIndex: Int, colorHex: String, label: String, a11y: String) {
    self.id = id
...
public struct MapModel: Sendable {
  ...
  public static func region(for points: [GeoPoint], minimumSpan: Double = 0.006) -> MapRegion {
    ...
    return MapRegion(
      center: GeoPoint(latitude: (minLatitude + maxLatitude) / 2, longitude: (minLongitude + maxLongitude) / 2),
      latitudeDelta: max(minimumSpan, (maxLatitude - minLatitude) * edgePadding),
      longitudeDelta: max(minimumSpan, (maxLongitude - minLongitude) * edgePadding)
    )
  }
