...
| 20 | エンジンへ渡す `inputIndex` は毎回いまの並びから押し直し、手入力の点の id(`manual-<番号>-<緯度5桁>-<経度5桁>`)も行を外すたびに新しい番号で作り直す —— ここまでは Web と同じ挙動。アプリだけがさらに、**その点に付いた編集(滞在時間・最終入場・日の指定・区間の手段・順の固定・外した場所)を新しい id へ一緒に動かす** | `ResolvedStop.inputIndex` は場所が決まった時刻の並びで凍り、`TripBuilder` は行と場所をまずその番号で突き合わせる —— 固定済みの行より前の行を外すと、以降の場所が 1 行ずつずれて貼り付く。Web も `removeResolvePlace` が `resolutionOverrides[].inputIndex` を詰め、`manualStopFromResolutionOverride` がその詰め直した番号から id を組み直すので、id の再構成そのものは Web と同じ。違うのは編集の扱い —— Web はその id をキーに持つ編集まで書き換えないので、古い id を指したまま宙に浮いて死ぬ。アプリは `restampManualPins` が編集の辞書も同じ変換で付け替えるので、外した後も旅行者の決めごとが残る |
...
Task 26 で `apple/tools/verify-kit.sh`(既定 `--parallel`)を 2 回連続で回した結果。数値のみ。

| | 値 |
| --- | --- |
| テスト総数 | **551 本すべて passed / `exit=0`**(Golden 11・不変条件 224・単体 315・smoke 1) |
| G1 golden | 500/500。修復対 200 組。原型 10 種 × 100 回の再現一致 |
| G2 パーサ | 500 件、F1 = 1.0(閾値 0.97)。recall / precision / 明示マーカー / 見出し / 日割り当ても 1.0 |
| G3 TS 照合 | golden 500 + builder 23 のフィールド差分 **0**。除外 61 行(4 種)は全行が現に差分を飲み込んでいる |
| 共有リンク | ベクタ 17/17 バイト同一、スコープ 68/68、復号のみのベクタ 29 |
| JSMath | 86,510 標本が V8 12.4.254.21 / Node 22.18.0 とビット一致(45,002 + 20,001 + 20,001 + 240 + 322 + 242 + 702) |
