...
export async function POST(request: Request) {
  ...
  const providerUnits = parsed.query ? 1 : 4;
  const access = paidProviderGateway.reserve(preflight, "hotel_recommendations", providerUnits);
  if (!access.ok) return paidApiDenialResponse(access, noStoreHeaders);
  const providerFetch = providerFetchWithParentSignal(request.signal);

  try {
    let candidates = await fetchGoogleHotelCandidates(parsed, apiKey, providerFetch);
    ...
    if (destination?.hotelFacts === "rakuten" && rakutenApplicationId && rakutenAccessKey) {
      try {
        const facts = await fetchRakutenHotelFacts(parsed.latitude, parsed.longitude, rakutenApplicationId, rakutenAccessKey, providerFetch);
